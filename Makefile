...
PKG_VERSION := $(VERSION_MAJOR).$(VERSION_MINOR).$(VERSION_MICRO)
BRANDING_VERSION := $(PKG_VERSION)
...
$(dsc_file): debian/changelog $(misc_files) $(po_files)
	@[[ "$(PKG_VERSION)" == "$(pkg_version)" ]] || { echo -e "Version number in debian/changelog ($(pkg_version)) does not match BRANDING.mak ($(PKG_VERSION)).\n(Run \"dch\")" >&2 ; exit 1 ; }
	$(MAKE) clean
	dpkg-source --build .
...
debian/changelog: $(misc_files)
	dch --controlmaint --no-auto-nmu --package $(BRANDING_SHORTNAME) --newversion $(PKG_VERSION)

.PHONY: deb_pkg_version
deb_pkg_version: debian/changelog
	{ n="0000" ; \
	[[ "_$(DEB_PKG_VERSION)" == "_$(pkg_version)" ]] && printf -v n "%04u" $$((10#$(DEB_BUILD_NUMBER)+1)) ; \
	echo -e "# This file is generated by Makefile\nDEB_PKG_VERSION := $(pkg_version)\nDEB_BUILD_NUMBER := $${n}" > .deb_pkg_version.mak ; }

.PHONY: release_deb
release_deb: deb_pkg_version clean
	$(MAKE) deb
